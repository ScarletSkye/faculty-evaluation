...
// Serve static assets from both the root and public folders cleanly
...
db.serialize(() => {
    ...
    // Seed Data: Normalizes user profile registry
    db.run(`INSERT INTO users (school_id, first_name, last_name, password_hash, role) VALUES 
        ...
        ('24a-0146', 'Mary Angel N.', 'Alcasoda', ?, 'student'),
        ('24a-0092', 'Schenly Raven G.', 'Bacani', ?, 'student'),
        ('24a-0069', 'Jhoanna Marie T.', 'Bañaria', ?, 'student'),
        ('24a-0007', 'Mely Angel A.', 'Matibag', ?, 'student'),
        ('24a-0018', 'Ada C.', 'Tan', ?, 'student')`, 
        [
        ...
        db.run(`INSERT INTO courses (course_code, course_title) VALUES 
            ...
            ('PATHFIT4', 'PathFit 4')`, function(err) {
                ...
                `, function(err) {
                    ...
                    // FIXED INJECTION RUNNER: Maps metrics seamlessly across every single active student account 
                    db.all(`SELECT user_id FROM users WHERE role = 'student'`, [], (err, studentRows) => {
                        if (err) return console.error(err);
                        db.all(`SELECT assignment_id FROM class_assignments`, [], (err, assignmentRows) => {
                            if (err) return console.error(err);
                            
                            const stmt = db.prepare(`INSERT OR REPLACE INTO evaluation_tracker (student_id, assignment_id, is_completed) VALUES (?, ?, 0)`);
                            studentRows.forEach(student => {
                                assignmentRows.forEach(assign => {
                                    stmt.run(student.user_id, assign.assignment_id);
                                });
                            });
                            stmt.finalize(() => {
                                console.log("🚀 SUCCESS: Schedule matrices mapped for all team members!");
                            });
                        });
                    });
                });
...
app.get('/faculty-dashboard.html', (req, res) => { res.sendFile(path.join(__dirname, 'index.html')); });
app.get('/admin-dashboard.html', (req, res) => { res.sendFile(path.join(__dirname, 'index.html')); });
app.get('/student-dashboard.html', (req, res) => { res.sendFile(path.join(__dirname, 'index.html')); });
